...
public class fragmentFavoritos extends Fragment {
    ...
    private PokemonFavoritoAdapter adapter;
    private PokemonDataSource dataSource;
    private List<Pokemon> pokemonFavoritos = new ArrayList<>();
    private Handler mainHandler;
    private ExecutorService executor;
    private boolean cargando = false;
    private int idPokemonActual = 0;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.fragmentofavoritos, container, false);
        recyclerView = view.findViewById(R.id.recyclerViewEquipos);
        TextView tvSinFavoritos = view.findViewById(R.id.tvSinFavoritos);
        adapter = new PokemonFavoritoAdapter(pokemonFavoritos, requireContext());
        recyclerView.setLayoutManager(new LinearLayoutManager(requireContext()));
        recyclerView.setAdapter(adapter);
        dataSource = new PokemonDataSource();
        tvSinFavoritos.setText("Conectando a base de datos");
        tvSinFavoritos.setVisibility(View.VISIBLE);
        adapter.setOnPokemonClickListener(new PokemonFavoritoAdapter.OnPokemonClickListener() {
            @Override
    ...
    private void cargarFavoritos() {
        ...
        executor.execute(new Runnable() {
            ...
            public void run() {
                try {
                    Thread.sleep(1000);
                    final ArrayList<Integer> favoriteIds = dataSource.obtenerFavoritos();
                    mainHandler.post(new Runnable() {
                        @Override
                        public void run() {
                            TextView tvSinFavoritos = getView().findViewById(R.id.tvSinFavoritos);
                            if (favoriteIds.isEmpty()) {
                                tvSinFavoritos.setText("No hay Pokémon favoritos");
                                tvSinFavoritos.setVisibility(View.VISIBLE);
                                cargando = false;
                                return;
                            }
                            ...
                            for (int i = 0; i < favoriteIds.size(); i++) {
                                int pokemonId = favoriteIds.get(i);
                                cargarDatosPokemon(pokemonId);
                            }
                        }
                    });
                } catch (Exception e) {
                    mainHandler.post(new Runnable() {
                        ...
                        public void run() {
                            TextView tvSinFavoritos = getView().findViewById(R.id.tvSinFavoritos);
                            tvSinFavoritos.setText("Error de conexión con MySQL");
                            tvSinFavoritos.setVisibility(View.VISIBLE);
                        }
                    });
                } finally {
    ...
    private void cargarDatosPokemon(int pokemonId) {
        executor.execute(new Runnable() {
            @Override
            public void run() {
                try {
                    Pokemon pokemon = sacarPokemonDeAPI(pokemonId);
                    if (pokemon != null) {
                        mainHandler.post(new Runnable() {
                            @Override
                            public void run() {
                                pokemonFavoritos.add(pokemon);
                                adapter.notifyItemInserted(pokemonFavoritos.size() - 1);
                            }
                        });
                    }
                } catch (Exception e) {
    ...
    private Pokemon sacarPokemonDeAPI(int id) {
        try {
